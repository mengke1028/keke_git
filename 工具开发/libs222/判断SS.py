...
def get_PL():
    """读取当前PL值"""
    for i in range(10):
        res = mk_OCR(676, 555, 807, 633, "c8c8c8|c7c7c7|c9c9c9|cecbce|cecfce", 0.99)
...
def Find_use(shengao=133):
    """定位角色位置,返回角色当前坐标"""
    resp_mxt = find_use_png(0, 145, 800, 529, path + r"\IMG\基础达标.bmp", 0.90)
    try:
        if resp_mxt != -1:
            X = resp_mxt[1]
            Y = resp_mxt[2]
...
def Find_exit():
    """打开菜单"""
    guanbi = FindString(690, 125, 718, 143, "关", [125, 124], 0.9)
    if guanbi != -1:
        print("找到 <关闭> 菜单关闭")
        return True
    else:
...
def get_user_map_HBL():
    ...
    if boss != -1:
        ...
        if use_map != -1:
            ...
            lingzhu = FindString(0, 0, 800, 600, "领主", [170, 169], 0.98)
            if lingzhu != -1:
                print("角色在boos房间")
                return 7
...
def get_shangdian():
    ...
    time1 = time.time() + 15
    while time1 > time.time():
        shangdian = FindString(107, 510, 152, 538, "出", [154, 153, 152, 164, 156], 0.98)
        if shangdian != -1:
            print("找到 <出售> ")
            return True
        else:
...
def get_use():
    ...
    while True:
        ...
        key_press("ESC")
        time.sleep(1)
        res = FindString(342, 495, 397, 518, "角", [196, 197, 200, 198, 195], 0.98)
        print(res)
        if res != -1:
            X = int(res[0])
            Y = int(res[1])
...
def Fenjie_SS2():
    ...
    fenjie = None
    while time1 > time.time():
        fenjie = FindString(0, 0, 584, 236, "奥尼尔", [153, 154, 152, 164, 156], 0.99)
        print('fenjie', fenjie)
        if fenjie == -1:
            print("没找到 <装备分解> ")
            time.sleep(1)
            ...
            print("找到 <修理分解机> ")
            xiuli = FindString(94, 75, 601, 515, "机", [153, 154, 152, 164, 156], 0.98)
            if xiuli != -1:
                X = int(xiuli[-2])
                Y = int(xiuli[-1])
                time.sleep(1)
    ...
    if fenjie != -1:
        ...
        click(1, 1)
        time.sleep(1)
        fenjie = FindString(94, 75, 601, 515, "分", [153, 154, 152, 164, 156], 0.9)
        if fenjie != -1:
            X = int(fenjie[-2])
            Y = int(fenjie[-1])
            click(X, Y)
...
def Fenjie_SS():
    ...
    fenjie = None
    while time1 > time.time():
        fenjie = FindString(94, 75, 601, 515, "备", [153, 154, 152, 164, 156], 0.99)
        print('fenjie', fenjie)
        if fenjie == -1:
            print("没找到 <装备分解> ")
            time.sleep(1)
...
def chushou2():
    ...
    if get_shangdian():
        ...
        key_press("NUM1")
        key_press('SPACE')
        # moren = find_image_in_region(163, 114, 684, 508, "img\确认进行.bmp", 0.99, 5)
        # if moren != -1:
        #     keyboard = Controller()
        #     print('开始输入确认进行')
        #     keyboard.type('确认进行')  # 输入价格
        #     time.sleep(0.2)
        #     key_press("ENTER")
        key_press('SPACE')
        # move('RIGHT')
...
def Fid_jinbi():
    ...
    resp_mxt = FindString(0, 100, 798, 566, "金", [254, 255], 0.98)
    if resp_mxt != -1:
        X = resp_mxt[-2]
        Y = resp_mxt[-1]
        return int(X), int(Y) + 10
...
def Fid_jinbi3():
    ...
    if -1 != ss:
        return ss
    resp_mxt = FindString(0, 200, 800, 561, "武", [105, 106], 0.97)
    if resp_mxt != -1:
        X = resp_mxt[-2]
        Y = resp_mxt[-1]
        return int(X) + 5, int(Y) + 5
...
def Fid_jinbi2():
    ...
    resp_mxt = FindString(0, 200, 800, 561, "间", [105, 106], 0.98)
    if resp_mxt != -1:
        print("找到时间引导石")
        X = resp_mxt[-2]
        Y = resp_mxt[-1]
...
def get_changwan():
    ...
    if '-1' not in str(changwan):
        ...
        time.sleep(1)
        flag = True
        for i in [(358, 232), (359, 296), (359, 363), (358, 169), (358, 168), (358, 168)]:
            click(i[0], i[1])
            if flag:
                key_press('SPACE')
                flag = False
...
def jintu_jingjileyuan():
    ...
    key_press(";")  # 打开工会
    for i in range(10):
        resp = find_image_in_region(xpos, ypos, width, length, path + r"\img\工会移动.bmp", 0.98)
        if resp == -1:
            time.sleep(1)
            continue
    print(resp)
    if resp:
        ...
        click(116, 40)  # 地区
        time.sleep(0.5)
        click(124, 143)  # 谢拉比尔号2
        time.sleep(0.5)
        click(246, 499)  # 荆棘乐园门口
        key_press("SPACE")
...
def back_home():
    ...
    while True:
        key_press("ESC")
        time.sleep(1)
        res = FindString(373, 426, 628, 565, "返回城镇", [196, 197, 195], 0.99)
        if res != -1:
            X = int(res[-2])
            Y = int(res[-1])
            click(X + 10, Y - 5)
