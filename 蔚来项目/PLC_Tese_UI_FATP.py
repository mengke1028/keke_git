...
def parse_db_address(address):
    ...
    try:
        ...
        byte_offset = int(db_part[1][-1])  # 起始位置 start
...
def on_connect_click():
    ...
    # 在这里添加连接到PLC的逻辑
    stop = False
    logging.info("连接按钮被点击，IP地址:" + str(plc_ip_entry.get()))
    ip = plc_ip_entry.get()
    rack = 0
    slot = 1
    # 创建并连接到PLC
    try:
        ...
        if plc_client.get_connected():
            ...
            # 创建连接状态检测线程
            # PLC_connect_th = threading.Thread(target=init)
            # PLC_connect_th.daemon = True  # 设置为守护线程
            # # 启动线程
            # PLC_connect_th.start()
...
xiaya_yuan.grid(row=2, column=2, padx=10, pady=5)

xiaya_gongzuo = tk.Label(root, bg='grey', text="下压气缸工作", padx=10, pady=5, width=12)
xiaya_gongzuo.grid(row=2, column=3, padx=10, pady=5)
...
# saoma_gongzuo.grid(row=3, column=2, padx=10, pady=5)

jiaju_daowei = tk.Label(root, bg='grey', text="夹具到位信号", padx=10, pady=5, width=12)
jiaju_daowei.grid(row=2, column=4, padx=10, pady=5)

baojing1 = tk.Label(root, bg='grey', text="报警信号1", padx=10, pady=5, width=12)
baojing1.grid(row=3, column=2, padx=10, pady=5)
